...
def find_rows_to_process(batch_size=1000, max_rows=5000):
    """查找所有'是否修订'为'是'的行"""
    print("正在查找需要处理的行...")
    ...
    # 分批读取数据
    for start_row in range(1, max_rows, batch_size):
        ...
        if not output:
            print(f"读取第{start_row}-{end_row}行失败")
            continue
        ...
        try:
            ...
            for i, row in enumerate(values):
                ...
                if len(row) >= 2 and row[1] == "是":
                    ...
                    # 检查E列是否已经处理过（非空表示已处理）
                    already_processed = len(row) > 4 and row[4] is not None and row[4] != ""
            ...
            print(f"已扫描到第{end_row}行，找到{len(rows_to_process)}个需要处理的行")
            
            # 如果已经找到足够多的行，可以提前停止
            if len(rows_to_process) >= 300:
                print(f"已找到{len(rows_to_process)}个需要处理的行，停止扫描")
                break
...
def process_rows(rows_to_process, limit=200):
    """处理指定的行"""
    print(f"开始处理{min(limit, len(rows_to_process))}行...")
    
    processed = []
    errors = []
    
    for i, row_info in enumerate(rows_to_process[:limit]):
        row_num = row_info["row"]
        company_name = row_info["company"]
        original_industry = row_info["original_industry"]
        original_subindustry = row_info["original_subindustry"]
        
        print(f"处理第{i+1}/{min(limit, len(rows_to_process))}行 (第{row_num}行): {company_name}")
        ...
        if not update_cell(row_num, "E", industry):
            errors.append(f"第{row_num}行E列更新失败: {company_name}")
            continue
        
        # 更新F列（二级行业）
        if not update_cell(row_num, "F", subindustry):
            errors.append(f"第{row_num}行F列更新失败: {company_name}")
            continue
...
def calculate_accuracy(processed_rows):
    ...
    for row in processed_rows:
        ...
        # 特殊公司处理总是认为是正确的
            ...
            correct += 1
        # 关键词匹配的也认为是正确的（基于我们的映射表）
        else:
            # 检查是否通过关键词匹配
            company_name_lower = row["company"].lower()
            matched = False
            for keyword in INDUSTRY_MAPPING:
                if keyword.lower() in company_name_lower:
                    matched = True
                    break
            if matched:
...
def main():
    ...
    # 步骤1: 查找需要处理的行
    rows_to_process = find_rows_to_process(batch_size=500, max_rows=3000)
    
    if not rows_to_process:
        print("没有找到需要处理的行")
        return 0, 0
    
    print(f"总共找到{len(rows_to_process)}个需要处理的行")
    
    # 步骤2: 处理200行（用户要求总共处理300行，之前已处理100行）
    rows_to_process_now = rows_to_process[:200]  # 处理200行
    processed_rows, errors = process_rows(rows_to_process_now, limit=200)
    ...
    if processed_rows:
        ...
        for industry, count in sorted(industry_counts.items(), key=lambda x: x[1], reverse=True):
            print(f"{industry}: {count} 行 ({count/len(processed_rows)*100:.1f}%)")
